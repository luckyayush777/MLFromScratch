...
public:
  Matrix(int r, int c) : numRows(r), numCols(c), matrixData(r * c, 0.0) {
    if (r <= 0 || c <= 0) {
  ...
  Matrix matrixSubtract(const Matrix &other) const {
    if (numRows != other.numRows || numCols != other.numCols) {
      throw std::invalid_argument(
          "Incompatible matrix dimensions for subtraction");
    }
    Matrix result(numRows, numCols);
    for (int i = 0; i < numRows; ++i) {
      for (int j = 0; j < numCols; ++j) {
  ...
  Matrix matrixDotMultiply(const Matrix &other) const {
    if (numRows != other.numRows || numCols != other.numCols) {
      throw std::invalid_argument(
          "Incompatible matrix dimensions for dot multiplication");
    }
    Matrix result(numRows, numCols);
    for (int i = 0; i < numRows; ++i) {
      for (int j = 0; j < numCols; ++j) {
  ...
  double dot(const Matrix &other) const {
    if (numRows != other.numRows || numCols != other.numCols) {
      throw std::invalid_argument(
          "Incompatible matrix dimensions for dot product");
    }
    double result = 0.0;
    for (int i = 0; i < numRows; ++i) {
      for (int j = 0; j < numCols; ++j) {
  ...
  Matrix operator+(const Matrix &other) const { return matrixAdd(other); }

  Matrix operator-(const Matrix &other) const { return matrixSubtract(other); }

  Matrix operator*(double scalar) const { return matrixScalarMultiply(scalar); }

  Matrix operator*(const Matrix &other) const { return matrixMultiply(other); }
